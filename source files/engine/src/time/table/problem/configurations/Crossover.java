...
public class Crossover implements Serializable {
    ...
    */
    //for EX3
    public Crossover(String i_cuttingPoints, String i_EnumCrossoverType, String i_OrientationType) throws Exception {
        m_CuttingPoint = Integer.parseInt(i_cuttingPoints);
        if (i_EnumCrossoverType.equals(type.AspectOriented.toString())) {
            m_EnumCrossoverType = type.AspectOriented;
        } else if (i_EnumCrossoverType.equals(type.DayTimeOriented.toString())) {
            m_EnumCrossoverType = type.DayTimeOriented;
        }
        if (m_EnumCrossoverType == type.AspectOriented) {
            try {
                setOrientationType(i_OrientationType);
            } catch (Exception e) {
                throw new Exception("Error in crossover's configuration.");
            }
        }
    }
    ...
    private void setOrientationType(String i_Configuration) throws Exception {
     //   int indexEquals = i_Configuration.lastIndexOf("=");
       // String classOrientationStr = i_Configuration.substring(indexEquals + 1);
        if (i_Configuration.equals(STUDY_CLASS)||i_Configuration.equals("STUDY_CLASS")) {
            m_OrientationType = STUDY_CLASS;
        } else if (i_Configuration.equals(TEACHER)||i_Configuration.equals("TEACHER")) {
            m_OrientationType = TEACHER;
        } else {
            throw new Exception("Couldn't find orientation type for crossover - AspectOriented: " + i_Configuration);
        }
    }

    public type getEnumCrossoverType() {
        return m_EnumCrossoverType;
    }

    public void setEnumCrossoverType(type i_EnumCrossoverType) {
        this.m_EnumCrossoverType = i_EnumCrossoverType;
    }
    ...
    public void setCuttingPoint(int m_CuttingPoint) {
        this.m_CuttingPoint = m_CuttingPoint;
    }
    ...
    public String toString() {
        if (m_OrientationType == "") {
            return "Name: " + this.m_EnumCrossoverType + ", Number of cutting points: " + this.m_CuttingPoint;
        }
        return "Name: " + this.m_EnumCrossoverType + ": " + m_OrientationType + "Type, Number of cutting points: " + this.m_CuttingPoint + System.lineSeparator();
