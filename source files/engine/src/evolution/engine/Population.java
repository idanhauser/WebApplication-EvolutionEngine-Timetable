...
public class Population<T> implements Serializable, Cloneable {
    ...
    public Population(Factory<T> i_Factory, EvolutionEngineInformation i_ProblemInfo, int i_geneListLength)
    {
        m_geneLength = i_geneListLength;


        m_PopSize= i_ProblemInfo.getInitialPopulation();
        m_Individuals = new ArrayList<>();

        //Create a first population pool
        for (int i = 0; i < m_PopSize; i++) {
            Individual ind = new Individual(i_Factory, i_ProblemInfo.GetRules() , i_geneListLength);
